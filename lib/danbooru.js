...
exports = module.exports = class Danbooru {
  ...
  async posts(options) {
    ...
    let json = await this.requestJson('posts', options)
    return json.map(post => new Post(this, post))
  }

  async post(id) {
    return new Post(this, (await this.requestJson('posts/' + id)))
  }

  async favorites(post_id, destroy) {
    if(!post_id) {
      let json = await this.requestJson('favorites', options)
      return json.map(post => new Post(this, post))
    } else {
